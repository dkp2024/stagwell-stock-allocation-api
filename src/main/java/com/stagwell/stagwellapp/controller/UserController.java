...
/*
    @PostMapping("user")
    public ResponseEntity<APIResponse> saveUser(@RequestBody SwmUserCredentials swmUserCredentials) {
        APIResponse apiResponse = new APIResponse();
        log.info("saveUser(-) in:{}", swmUserCredentials.toString());
        try {
            userService.saveUser(swmUserCredentials);
            apiResponse.setMessage("User: %s saved successfully!".formatted(swmUserCredentials.getUserName()));
            apiResponse.setStatusCode(Constants.SUCCESS);
            return new ResponseEntity<>(apiResponse, HttpStatus.OK);
        } catch (Exception e) {
            log.info(e.getMessage());
            apiResponse.setMessage("User: %s not saved with error: %S".formatted(swmUserCredentials.getUserName(), e.getMessage()));
            apiResponse.setStatusCode(Constants.FAILED);
    ...
    @PostMapping("user")
    //public ResponseEntity<UserLeastInfoDto> validateAndSave(@PathVariable String email ) {
            ...
            if (!ObjectUtils.isEmpty(userCredentials)) {
                ...
                if (!ObjectUtils.isEmpty(userInfoList)) {
                    ...
                    if(Arrays.asList(validateUserDto.getRoles()).contains(swmUserInfo.getUserType())){
                       ...
                       String cookieDATA= encryptedUserId+"_"+encryptedEmail;
